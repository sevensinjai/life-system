...
- The offline-first iOS store now applies the same 100 × level^1.5 threshold curve as the default backend tuning, persists child nodes in their parent branch, preserves descendants during updates, and atomically saves practice media bytes with direct and ancestor awards.
